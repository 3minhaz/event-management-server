...
const createPackage = asyncHandler(async(req,res) =>{
    ...
    const {package_name,image_link,description,categories,email} = req.body;
    if(!email){
        res.status(401);
        throw new Error("User not Autorized!");
    }
    if (!package_name || !image_link || !description || !categories) {
        res.status(400);
        throw new Error("Please Fill all Feilds");
    }else {
        const package = new Package({ package_name, user:req.user._id, image_link, description,categories });
